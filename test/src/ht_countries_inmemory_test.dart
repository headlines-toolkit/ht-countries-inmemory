...
void main() {
  ...
  final countryAF = Country(
    id: 'af-id',
    isoCode: 'AF',
    name: 'Afghanistan',
    flagUrl: 'http://example.com/af.png',
  );
  final countryAL = Country(
    id: 'al-id',
    isoCode: 'AL',
    name: 'Albania',
    flagUrl: 'http://example.com/al.png',
  );
  final countryDZ = Country(
    id: 'dz-id',
    isoCode: 'DZ',
    name: 'Algeria',
    flagUrl: 'http://example.com/dz.png',
  );
  final countryAS = Country(
    id: 'as-id',
    isoCode: 'AS',
    name: 'American Samoa',
    flagUrl: 'http://example.com/as.png',
  );
  final countryAD = Country(
    id: 'ad-id',
    isoCode: 'AD',
    name: 'Andorra',
    flagUrl: 'http://example.com/ad.png',
  );
  ...
  group('HtCountriesInMemoryClient Constructor', () {
    test(
      'initializes with default kInitialCountriesData when no map is provided',
      () {
        ...
        expect(
          client.countries.containsKey('US'),
          isTrue,
        ); // Assuming US is in default data
        expect(client.countries['US']?.name, kInitialCountriesData['US']?.name);
        expect(
          client.countries['GB']?.flagUrl,
          kInitialCountriesData['GB']?.flagUrl,
        );
      },
    );
    ...
    test('initializes with provided initialCountries map', () {
      final client = HtCountriesInMemoryClient(
        initialCountries: specificTestData,
      );
      expect(client.countries.length, specificTestData.length);
      expect(
        client.countries,
        equals(specificTestData),
  ...
  group('HtCountriesInMemoryClient Methods', () {
    ...
    group('fetchCountries', () {
      test(
        'returns initial countries sorted by isoCode when no params',
        () async {
      ...
      test('returns limited number of countries', () async {
        ...
        expect(
          ...
          sortedIsoCodesSpecific.sublist(0, 2),
        ); // AD, AF
      });
      ...
      test(
        'returns empty list if startAfterId is the ID of the last item',
        () async {
          // Start after Algeria (dz-id)
          final countries = await client.fetchCountries(
            limit: 3,
            startAfterId: countryDZ.id,
          );
      ...
      test(
        'returns empty list if effective startIndex exceeds list length',
        () async {
      ...
      test('handles pagination correctly at boundaries using IDs', () async {
        ...
        countries = await client.fetchCountries(
          limit: 2,
          startAfterId: lastFetchedId1,
        );
    ...
    group('createCountry', () {
      test('successfully adds a new country', () async {
        final newCountry = Country(
          ...
          isoCode: 'GB',
          name: 'United Kingdom',
          flagUrl: 'http://example.com/gb.png',
        );
      ...
      test(
        'throws CountryCreateFailure if country with isoCode already exists',
        () async {
    ...
    group('updateCountry', () {
      test('successfully updates an existing country', () async {
        final updatedCountry = Country(
          id: countryAL.id,
          isoCode: 'AL',
          name: 'Albania Updated',
          flagUrl: 'http://example.com/al_updated.png',
        );
        ...
        expect(
          ...
          specificTestData.length,
        ); // Count shouldn't change
      });

      test(
        'throws CountryNotFound if country isoCode does not exist',
        () async {
          final nonExistingCountry = Country(
            id: 'xx-id',
            isoCode: 'XX',
            name: 'Non Existent',
            flagUrl: 'http://example.com/xx.png',
          );
          expect(
